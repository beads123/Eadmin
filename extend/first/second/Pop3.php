...
class Pop3 
{
    ...
    public function to($server="192.100.100.1",$port=110,$time_out=5)
    {
  ...
  //下面介绍的几个方法则可以按照前述收取信件的对话去理解，因为有关的内容已经在前面做了说明，因此下面的方法不做详细的说明，请参考其中的注释： 
	...
	public function listmail($mess=null,$uni_id=null) //对应的是LIST命令，取得每个邮件的大小及序号。一般来说用到的是List命令，如果指定了$uni_id ，则使用UIDL命令，返回的是每个邮件的标识符，事实上，这个标识符一般是没有什么用的。取得的各个邮件的大小返回到类的内部变量mail_list这个二维数组里。 
	{ 
		if($this->state!="TRANSACTION") 
		{ 
			$this->err_str="还没有连接到服务器或没有成功登录";
	 ...
	 *默认值$line=-1，即取回所有的邮件内容
	 *取得的内容存放到内部变量$head，$body两个数组里
	...
	*/
	...
	public function dele($num) // 删除指定序号的邮件，$num 是服务器上的邮件序号 
	{
		if($this->state!="TRANSACTION"){
